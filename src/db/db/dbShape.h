...
// [[ZH-BEGIN]]
// ============================================================================
//  dbShape.h —— db::Shape：版图图形的**句柄（代理）**
// ============================================================================
//
// 【★ 新人最容易搞错的一件事】
//   db::Shape **不是**一个图形对象，而是一个**指向图形的句柄**。
//   上面英文注释已说清要点，翻译成使用建议：
//     · 拷贝 Shape 很便宜（只拷句柄）；
//     · 但两份拷贝指向**同一个**图形，改一个两个都“变”；
//     · 原图形被删除后，句柄变成**悬空**（无效），且不报错；
//     · Shape 可以是“空 (null)”，使用前应判有效性。
//   （更详细的四条行为说明见下面 class Shape 处的注释块。）
//
// 【为什么用句柄而不是直接给图形对象】
//   一个 cell 的一层里可能有上亿个图形。若遍历时每次都构造完整对象
//   （例如拷贝整个 polygon 的顶点），开销无法接受。
//   句柄让“遍历 + 选择性处理”变得廉价：只有确实需要几何时才展开它。
//
// 【★ 一个 Shape 可以是四种形态之一】
//     box       矩形
//     polygon   多边形（可能带孔）
//     path      路径（折线 + 宽度）
//     text      文本（**不是几何** —— 无面积、不参与布尔运算）
//   因此本文件大量出现 `if (m_type == Polygon) ... else if ...`
//   —— 这叫**类型标签派发**，是句柄模式的典型实现。
//   实用影响：遍历时必须处理四种情况（或至少知道有 text 这种“非几何”）；
//   例如“求面积”时 text 的贡献是 0，而不是报错。
//
// 【与其它核心类型的关系（这是理解整个 db 模块的骨架）】
//     db::Shape   ← 单个图形的句柄        （本文件）
//     db::Shapes  ← 一层上的所有图形容器  （dbShapes.h）
//     db::Cell    ← 一个单元：多层 Shapes + 对其它 cell 的引用（dbCell.h）
//     db::Layout  ← 整个数据库：多个 Cell + 层定义（dbLayout.h）
//   典型遍历路径：Layout → Cell → Shapes（按层）→ Shape（逐个）
//
// 【本文件其它内容】
//     generic_polygon_edge_iterator  统一的多边形边迭代器
//     Shape 的各类转换/查询方法（box()/polygon()/path()/text()/bbox()/area()…）
//     带属性的变体（ShapeWithProperties）
// [[ZH-END]]
/**
 ...
 */ 
// [[ZH-BEGIN]]
// 功能：★ **版图图形的句柄（代理对象）** —— 指向一个真实图形，但不拥有它。
//
// 【★★ 四条必须记住的行为（全部来自上面英文注释，都是实际踩坑点）】
//   1) **不拷贝图形**：拷贝 Shape 只是拷贝句柄。
//      → 所以可以廉价地传递/存储 Shape，但要注意下面第 2、3 条。
//   2) **拷贝后仍指向同一个图形**：两份句柄指向同一份数据。
//      通过一份修改几何，另一份也会“看到”改动（不是快照）。
//   3) **可能失效（悬空）**：若原图形被删除（如所在 Shapes 被清空或重排），
//      句柄就变成无效引用 —— **不会自动报错**，访问后果未定义。
//      ★ 因此：**不要长期保存 Shape**。跨修改操作保存的句柄应视为可能失效，
//        需要时重新通过遍历或索引获取。
//   4) **可以为 null**：即“无效引用”。使用前应判断有效性。
//
// 【一个 Shape 是四种形态之一】box / polygon / path / text。
//   类型由内部的类型标签决定（见文件头的说明）；
//   因此访问具体几何前需先判类型或用对应的查询方法。
//
// 【典型遍历骨架】
//     for (auto s = shapes.begin (); ! s.at_end (); ++s) {
//       if (s->is_polygon ())   { auto p = s->polygon ();  ... }
//       else if (s->is_path ()) { auto p = s->path ();     ... }
//       else if (s->is_box ())  { auto b = s->box ();      ... }
//       else                    { /* text 等：注意 text **不是**几何 */ }
//     }
//   ★ 具体方法名以类内声明为准（如 is_polygon/polygon/is_box/box/is_text/text）；
//     上面展示的是遍历与分派的思路，不是可照抄的完整代码。
//
// 【为什么本类看起来“没有几何”】
//   因为它不是几何类型，而是“指向几何的指针”。
//   真正的几何类型是 db::Box / db::Polygon / db::Path / db::Text。
//   把 Shape 当成 Polygon 的别名会导致理解偏差（见 dbPolygon.h 末尾的提醒）。
// [[ZH-END]]
