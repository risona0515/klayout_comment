 ...
 */
// [[ZH-BEGIN]]
// 功能：★ **空区域实现** —— 表示“没有多边形”，但**是一个合法对象**。
//
// 【为什么需要一个专门的“空”实现（而不是用 0 指针）】
//   因为这样可以让空值也**参与全部运算而不需要到处判空**：
//     · 空区域 & 任何区域      → 空
//     · 空区域 | 某区域        → 某区域
//     · 空区域的面积           → 0（而不是异常/崩溃）
//   这就是所谓的 **Null Object 模式**：用一个“行为良定义的空对象”
//   取代空指针，从而消除大量分支判断。
//   ★ 这与 EmptyRegion 不存任何数据的事实配合，使“空”几乎零成本。
//
// 【典型出现位置】
//   · Region 的默认/初始状态；
//   · 某些运算的空结果（如两个不相交区域的交集）；
//   · 被清空后的区域。
//
// 【与其他实现的关系】
//   它**直接**继承 RegionDelegate（不经 AsIfFlatRegion）——
//   因为空区域无需任何“平铺”逻辑，所有操作都可直接给出平凡答案。
// [[ZH-END]]
