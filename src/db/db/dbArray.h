...
// [[ZH-BEGIN]]
// ============================================================================
//  db::array —— ★ “数组引用”的门面（版图里大量图形/实例的紧凑表示）
// ============================================================================
//
// 【它解决什么问题（先理解动机，语法就简单了）】
//   版图里经常出现**规则重复**的图形或实例，例如：
//       · 一个 100×200 的接触孔阵列（每个都是同一个正方形）；
//       · 一条存储阵列，同一单元按行列重复。
//   若把每个副本都单独存成一个图形，顶点数据会被重复上万遍。
//   因此引入数组：**只存一份对象 + 描述重复规则（步距/行列数）**。
//   上面英文的第 3 段明确说了它的用途：“用于创建 shape 与 instance 数组”。
//
// 【★ 结构：门面 + 四个内部实现（本文件的主干）】
//   db::array<Obj, Trans>          ← 门面（你实际用的类型）
//        └─ mp_base 指向一个 basic_array（描述“怎么重复”）
//             有四个特化，分别对应四种重复方式：
//               regular_array              规则阵列（给两个步距向量 a、b + 行列数）
//               regular_complex_array      规则阵列 + 复杂变换（任意角度/缩放）
//               iterated_array             迭代阵列（用迭代器逐个给出位移）
//               iterated_complex_array     迭代阵列 + 复杂变换
//   ★ 为什么分四个：因为“复杂变换”（含任意角度/缩放）需要在每个副本上
//     额外存变换信息，与只用简单变换的情形在存储上不同。
//
// 【★★ shape 数组 与 instance 数组的根本差别（上游特别强调，很关键）】
//   英文最后一段说明了这一点，展开：
//     · **shape 数组**（图形阵列）：
//         阵列本身可以直接变换那个图形；
//         因此**不能**使用复杂变换型阵列（否则图形形态会变，不符合“同一图形重复”）。
//     · **instance 数组**（实例阵列，即“单元阵列”）：
//         **不能**变换被引用的对象本身（那个单元的内容是共享的！），
//         所以变换的**每一个方面都必须存进阵列里**。
//         因此实例阵列会用到复杂变换特性。
//   ★ 这条差别是理解“为什么同一个 array 类要支持两种用法”的关键。
//     也解释了 GDS/OASIS 里 AREF 的语义约束。
//
// 【迭代方式：给的是位移，不是对象副本】
//   上面英文说明：访问实例靠**迭代器**，迭代器给出各个副本的
//   **位移向量 (displacement)**，并且**可以按矩形区域过滤**。
//   于是遍历阵列不会构造出一堆对象，内存与时间都很省。
//   ★ 这同样意味着：迭代得到的是“位置/位移”，需要配合原始对象才构成一个副本。
//
// 【本文件其余内容】
//   basic_array_iterator / 各迭代器特化   四种阵列的迭代实现
//   ArrayRepository                     阵列基对象的去重仓库（同一阵列可共享）
//   basic_array 及其四个特化            阵列的基类与具体实现
//   compute_result_trans                “变换与阵列变换如何组合”的辅助模板
// [[ZH-END]]
