...
// [[ZH-BEGIN]]
// ============================================================================
//  tl::Variant —— ★★ 通用“变体”值类型（全库的“任意值”容器）
// ============================================================================
//
// 【它是什么】
//   一个能**装下多种类型**的单一类型：整数（各种宽度）、浮点、字符串、
//   布尔、nil（空）、以及**由 Variant 组成的列表**。
//   上面英文注释列举了基本能力；“A basic variant type”即“基础变体类型”。
//
// 【★★ 为什么整库都依赖它（实用价值最大的理解点）】
//   它充当**跨语言/跨层的公共数据格式**，是多个子系统的交汇点：
//     · **脚本 ↔ C++**：Ruby/Python 传入的值统一转成 Variant 处理；
//     · **属性系统**：属性的名字与值都是 Variant
//       （见 dbPropertiesRepository.h —— 那里说“名字/值”其实就是本类型）；
//     · **XML/配置**：读写配置时用 Variant 表示任意配置项的值；
//     · **PCell 参数**：参数值（可为数字/字符串/枚举）用 Variant 承载。
//   ★ 所以：看到某个 API 参数是 Variant，就意味着“**这里可以传任意类型**”，
//     而内部分派逻辑会按实际类型走不同分支。
//
// 【★★ 类型标签与“当前是哪种类型”】
//   下面 enum type 列出所有可能的类型（t_nil / t_bool / t_char / t_int /
//   t_double / t_string / t_list ...）。
//   ★ 关键机制：Variant **记住自己当前是哪种类型**，因此：
//     · is_* () 可查询当前类型（如 is_string ()）；
//     · to_* () / as_* () 可取具体值（可能需要转换）；
//     · 不同 Variant 的**比较与判等依赖类型**（1 与 \"1\" 不是同一个值）。
//   ★ 实用坑：读属性值时应先确认类型，不要假定一定是字符串或数字 ——
//     属性的类型取决于**写入时**的类型（脚本里写什么就是什么）。
//
// 【★ t_nil 是“空值”，不是 0】
//   变体有自己的空状态（nil），表示“没有值”。
//   这与“值为 0 / 空字符串”**不同**，是第三态。
//   典型用途：属性未设置、可选参数未提供、函数返回“无”。
//
// 【★ 列表支持的意义】
//   变体可以装“其它变体的列表”，因此**天然能表达任意嵌套结构**
//   （数组/树），这就是为什么它能同时胜任属性值、PCell 参数与配置项。
//
// 【与其它 tl 类型的关系】
//   tl::Variant   ← 任意值（本文件，最有名的 tl 类型）
//   tl::Extractor ← 文本解析器（它能解析出 Variant，见 dbPoint.cc 的说明）
//   tl::String    ← 字符串工具（Variant 的字符串分支用它）
//
// 【读本文件的建议】
//   方法极多但规律性强：is_xxx / to_xxx / as_xxx 三族 + 类型标签 + 列表操作。
//   先看 enum type 与构造/类型查询，再看转换，最后看列表。
// [[ZH-END]]
class TL_PUBLIC Variant
{
public:
  // [[ZH]] 功能：枚举**所有可能承载的类型**。
  // [[ZH]] ★ 由此可见它能装的整数类型很细（char/short/int/long/longlong 各有
  // [[ZH]]       有无符号版本）—— 这是为了**忠实保留脚本/文件里的原始类型**，
  // [[ZH]]       避免在转换中悄悄改变数值语义（如无符号变有符号）。
  // [[ZH]] t_nil 是**空值**（第三态），不是 0。
  // [[ZH]] t_list 表示“装的是其它 Variant 的列表”（因此支持嵌套）。
  enum type { 
    t_nil, 
    t_bool, 
    t_char, 
